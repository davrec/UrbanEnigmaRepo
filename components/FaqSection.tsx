...
export function FAQSection() {
  const [openIndex, setOpenIndex] = useState(null); // Tiene traccia di quale card è aperta
    ...
    <div className="w-full pb-32 bg-bgChiaro text-secondary">
      <div className="container mx-auto flex flex-col items-center w-[80%] sm:w-[60%]">
        ...
        <div className="flex flex-col gap-8 w-full">
          {faqs.map((faq, index) => (
            <div key={index} className="bg-bgScuro p-6 rounded-lg shadow-lg">
              <div className="flex justify-between items-center">
                <h4 className="text-2xl font-semibold text-white">{faq.question}</h4>
                <button
